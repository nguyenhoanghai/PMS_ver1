using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace DuAn03_HaiDang.Enum
{
    public class eFile
    {
        public const string NSCHUYEN = "NSCHUYEN";
        public const string NSCCUM = "NSCUM";
        public const string NS_CHUYEN_THEO_GIO_MrTri = "NSCHUYENHANGGIO";
        public const string NS_CHUYEN_THEO_GIO_MDG = "NSCHUYENHANGGIO_MDG";
        public const string NS_CHUYEN_THEO_GIO_THIENSON = "NSCHUYENHANGGIO_2";
        public const string NS_CHUYEN_THEO_GIO_SONHA = "NSCHUYENHANGGIO_SH"; 
        public const string NS_CHUYEN_THEO_GIO_SONHA_NgayCu = "NSCHUYENHANGGIO_SH_YES";
        public const string NSCHUYEN_HOANGGIA  = "NSCHUYEN_HOANGGIA"; 
        public const string Chart_KCSInHour = "CHART_KCSINHOUR";
        public const string Chart_TCInHour = "CHART_TCINHOUR";
        public const string Chart_ERRORInHour = "CHART_ERRORINHOUR";
        public const string Chart_KCSInDay = "CHART_KCSINDAY";
        public const string Chart_TCInDay = "CHART_TCINDAY";
        public const string Chart_ERRORInDay = "CHART_ERRORINDAY"; 
    }
}
