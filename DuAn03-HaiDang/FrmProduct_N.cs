    ...
    public partial class FrmProduct_N : Form
    {
        ...
        private void gridView_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                //if (Id != 0 && string.IsNullOrEmpty(gridView.GetRowCellValue(gridView.FocusedRowHandle, "TenSanPham").ToString()))
                //    MessageBox.Show("Vui lòng nhập tên sản phẫm.", "Lỗi nhập liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //else if (Id != 0 && string.IsNullOrEmpty(gridView.GetRowCellValue(gridView.FocusedRowHandle, "DonGia").ToString()))
                //    MessageBox.Show("Vui lòng nhập đơn giá.", "Lỗi nhập liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //else if (Id != 0 && string.IsNullOrEmpty(gridView.GetRowCellValue(gridView.FocusedRowHandle, "DonGiaCM").ToString()))
                //    MessageBox.Show("Vui lòng nhập đơn giá CM.", "Lỗi nhập liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //else if (Id != 0 && string.IsNullOrEmpty(gridView.GetRowCellValue(gridView.FocusedRowHandle, "ProductionTime").ToString()))
